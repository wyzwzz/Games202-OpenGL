...
private:
    virtual void initialize() {
        ...
        loadEnvLight("asset/environment.hdr");
//        loadEnvLight("asset/small_hangar_01_4k.hdr");
    ...
    virtual void frame() {
        ...
        if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize)){
            ...
            if(ImGui::RadioButton("InterReflect",light_mode == LightMode::InterReflect)){
                ...
                ImGui::InputInt("Max Inter Reflect Depth",&max_inter_reflect_depth);
                generateMeshSHCoefs();
            }

            if(ImGui::SliderInt("SH Max Degree",&sh_degree,0,MaxSHDegree)){
                generateEnvSHCoefs();
                generateMeshSHCoefs();
            }
        ...
        shader.set_uniform_var("LightSHCount",sh::get_coef_count(sh_degree));
        shader.set_uniform_var("Model",mat4::identity());
...
private:
    ...
    void loadEnvLight(const std::string& filename);

    std::vector<vec3f> computeEnvSHCoefs(const wzz::texture::image2d_t<color3f>& env,
                                         int max_degree,int sample_count);

    void rotateEnvSHCoefs(const mat3& rot,const std::vector<vec3f>& env_coefs);

    void generateMeshSHCoefs();

    void generateEnvSHCoefs();
    ...
    void updatePRT(bool update_mesh,bool update_env);
private:
    static constexpr int MaxSHDegree =  4;
    static constexpr int MAXSHCount = wzz::math::sh::get_coef_count(MaxSHDegree);

    int sh_degree = 4;
    int sample_count_per_vertex = 1024;
    int light_sample_count = 1 << 20;
    float default_albedo = 0.7f;
    int max_inter_reflect_depth = 5;
    ...
    wzz::texture::image2d_t<color3f> env_light_;

    LightSHInfo light_sh_coefs;
    std::vector<vec3f> light_sh_coefs_arr;
...
void PRTApp::loadModel(const std::string& filename,const std::string& mtl){
    ...
    for(auto& mesh:model->meshes){
        ...
        if (mesh.material != -1) {
            ...
            m.albedo_tex.initialize_format_and_data(1,GL_RGB8,wzz::texture::image2d_t<wzz::math::color3b>(
                    2,2,wzz::math::color3b(uint8_t(255 * default_albedo))));
        }
    ...
    for(int i = 0; i < count; ++i){
        ...
        auto coefs = computeMeshSHCoefs(*bvh,mesh.pos,mesh.normal,mesh.tex_coord,model->materials,-1,
                           sh_degree,sample_count_per_vertex,light_mode);
    ...
    bvh_ = std::move(bvh_);
    LOG_INFO("total mesh sh coefs count: {}",total_count);
    LOG_INFO("load model ok");
}
...
void PRTApp::generateMeshSHCoefs() {
    ...
    for(int i = 0; i < count; ++i){
        ...
        auto coefs = computeMeshSHCoefs(*bvh_,mesh.pos,mesh.normal,mesh.tex_coord,model_->materials,-1,
                                        sh_degree,sample_count_per_vertex,light_mode);

        // setup gl buffer
        auto& draw_mesh = draw_meshes[i];
        total_count += coefs.size();
        draw_mesh.vertex_sh_ssbo.set_buffer_data(reinterpret_cast<const float*>(coefs.data()),
                                                        coefs.size() * sizeof(vec3f),
                                                        GL_DYNAMIC_STORAGE_BIT);
    }
    LOG_INFO("total mesh sh coefs count: {}",total_count);
...
static void computeVertexSHCoefsInterReflect(const BVHAccel& bvh,
    ...
    for(int i = 0; i < max_depth; ++i){
        ...
        vec3f brdf = vec3f(default_albedo);
                                              ...
                                              int max_degree,int sample_count_per_v, LightMode mode)
{
    ...
    auto albedo = material_id == -1 ?
                  wzz::texture::image2d_t<vec3f>(2,2,vec3f(default_albedo))
    ...
    parallel_forrange(0,vertex_count,[&](int thread_index,int vertex_id){
        ...
        for(int i = 0; i < sample_count_per_v; ++i){
            ...
            if(mode == LightMode::NoShadow){
                ...
                computeVertexSHCoefsInterReflect(bvh,ray,coef,materials,default_albedo,
                                                 max_inter_reflect_depth,sh_coef_count,res,[&](){
...
void PRTApp::generateEnvSHCoefs() {
    light_sh_coefs_arr = computeEnvSHCoefs(env_light_,sh_degree,light_sample_count);

    for(int i = 0; i < MAXSHCount; ++i)
        light_sh_coefs.light_sh_coefs[i] = vec4(0);

    for(int i = 0; i < sh::get_coef_count(sh_degree); i++)
        light_sh_coefs.light_sh_coefs[i] = vec4f(light_sh_coefs_arr[i].x,
                                                 light_sh_coefs_arr[i].y,
                                                 light_sh_coefs_arr[i].z,0.0);
    light_sh_buffer.set_buffer_data(&light_sh_coefs);
    LOG_INFO("light sh coefs ok");
}

void PRTApp::loadEnvLight(const std::string &filename) {
    env_light_ = wzz::texture::image2d_t<color3f>(wzz::image::load_rgb_from_hdr_file(filename));
...
//        LOG_INFO("_phi _theta: {} {}",_phi,_theta);
...
void PRTApp::rotateEnvSHCoefs(const mat3 &rot, const std::vector<vec3f> &env_coefs) {
