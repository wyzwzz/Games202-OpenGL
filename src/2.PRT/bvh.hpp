...
public:
    ...
    void build(std::vector<RC<Primitive>> prims) {
        ...
        LOG_DEBUG("bvh tree build node count: {}", total_nodes_count);
    }
            ...
            if (centroid_bounds.high[dim] == centroid_bounds.low[dim]) {
                ...
                if (primitives_count <= SAH_PRIMITIVES_THRESHOLD) {
                    ...
                    for (int i = 0; i < N_BUCKETS - 1; ++i) {
                        ...
                        cost[i] =
                                1 + (l_count * lb.surface_area() + r_count * rb.surface_area()) / bounds.surface_area();
                    }
                ...
                node->init_interior(dim, recursive_build(primitive_infos, ordered_prims, start, mid, total_nodes_count,
                                                         arena),
                                    recursive_build(primitive_infos, ordered_prims, mid, end, total_nodes_count,
                                                    arena));
            }
        }
        return node;
